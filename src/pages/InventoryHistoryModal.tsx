...
import { 
  X, History, ArrowDownRight, ArrowUpRight, 
  Undo2, AlertTriangle, SlidersHorizontal, 
  Truck, PackageCheck, FileText, Package 
} from 'lucide-react';
...
export function InventoryHistoryModal({ inventory, onClose }: InventoryHistoryModalProps) {
  // ĐÃ SỬA (Bug 2): Thêm state phân trang
  const [page, setPage] = useState(0);
  const PAGE_SIZE = 10;

  // ĐÃ SỬA (Bug 2): Truyền page và size vào useQuery
  ...
  // Xử lý dữ liệu Page trả về từ Spring Boot
  const transactions = data?.content || [];
  const totalPages = data?.totalPages || 0;
  const totalElements = data?.totalElements || 0;

  // ĐÃ SỬA (Bug 4): Thêm case RETURN_TO_DEFECT
  const getTransactionInfo = (type: string) => {
    const map: Record<string, any> = {
      ...
      'RETURN_TO_DEFECT': { label: 'Trả về kho lỗi', color: 'text-red-700 bg-red-50 border-red-200', icon: AlertTriangle }, // <-- ĐÃ SỬA TẠI ĐÂY
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm animate-fade-in">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-5xl max-h-[90vh] flex flex-col overflow-hidden animate-slide-up">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500 font-medium">
                {inventory.product?.name} <span className="text-gray-300 mx-1">|</span> SKU: {inventory.product?.sku || 'N/A'}
              </p>
            </div>
          </div>
          <button 
          ...
          {isLoading ? (
            ...
            <div className="py-12">
              <EmptyState 
                ...
                description="Sản phẩm này chưa có biến động nào trong kho." 
              />
            </div>
          ) : (
                  ...
                  {transactions.map((txn: any) => {
                    ...
                    return (
                      <tr key={txn.id} className="hover:bg-gray-50/80 transition-colors">
                        <td className="py-3 px-4 text-center text-gray-600 font-medium text-[13px]">
                        ...
                        <td className="py-3 px-4 text-gray-600 text-[13px] max-w-xs truncate" title={txn.note || ''}>
                          {txn.note || '-'}
                        </td>
                      </tr>
                    );
        ...
        {/* ĐÃ SỬA (Bug 2): Footer Modal có Phân trang */}
        <div className="p-4 border-t bg-gray-50 flex items-center justify-between rounded-b-2xl">
          <div className="text-sm text-gray-500 font-medium">
            Tổng cộng: <span className="font-bold text-gray-800">{totalElements}</span> giao dịch
          </div>
          {totalPages > 1 && (
            <Pagination 
              page={page} 
