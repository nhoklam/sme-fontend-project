...
import {
  ...
  ArrowLeftRight, ShoppingBag, FolderTree,
  ClipboardList, Tag, Receipt, History, UserCog, Building2, X,
} from 'lucide-react';
...
const navItems = [
  ...
  },
  {
    label: 'QUẢN LÝ',
    items: [
      { icon: LayoutDashboard, label: 'Tổng quan & AI',  to: '/dashboard',   roles: ['ROLE_ADMIN', 'ROLE_MANAGER', 'ROLE_CASHIER'] },
      ...
      { icon: Tag,            label: 'Khuyến mãi',      to: '/promotions',  roles: ['ROLE_ADMIN', 'ROLE_MANAGER'] },
    ],
  },
  {
    label: 'KHO HÀNG',
    items: [
      ...
      { icon: ClipboardList,  label: 'Kiểm kê kho',     to: '/stock-takes',     roles: ['ROLE_ADMIN', 'ROLE_MANAGER'] },
    ],
  },
  {
    label: 'ĐỐI TÁC',
...
export default function Sidebar({ isOpen, onClose }: { isOpen?: boolean; onClose?: () => void }) {
  ...
  return (
    <aside className={cn(
      'fixed inset-y-0 left-0 z-50 flex flex-col bg-white border-r border-slate-200 w-[220px] flex-shrink-0',
      ...
      <div className="flex items-center justify-between px-4 h-12 md:h-14 border-b border-slate-100 shrink-0">
        <div className="flex items-center gap-2.5">
          ...
          <p className="font-black text-slate-900 text-[14px] leading-tight tracking-tight">Bookly</p>
        </div>
        ...
        {navItems.map((group) => {
          ...
          return (
            <div key={group.label} className="space-y-1">
              <p className="px-3 text-[9px] font-bold text-slate-400 uppercase tracking-widest mb-1.5">{group.label}</p>
              {visible.map((item) => {
                if (item.to === '/pos') {
                  return (
                ...
                return (
                  <NavLink key={item.to} to={item.to}
                    className={({ isActive }) => cn(
                      ...
                      isActive ? 'text-indigo-700 bg-indigo-50 font-bold' : 'text-slate-600 hover:text-slate-900 hover:bg-slate-50 font-medium'
                    )}>
                    {({ isActive }) => (
                      <>
      ...
      <div className="p-3 border-t border-slate-100 shrink-0 bg-slate-50/50">
        <div className="flex items-center justify-between bg-white border border-slate-200 p-2 rounded-lg shadow-sm">
          ...
          <button onClick={handleLogout}
            className="w-7 h-7 flex items-center justify-center text-slate-400 hover:text-rose-600 hover:bg-rose-50 rounded-md transition-colors" title="Đăng xuất">
